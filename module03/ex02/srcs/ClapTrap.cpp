...
/*   Updated: 2022/10/19 10:44:14 by vrigaudy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ClapTrap.hpp"

ClapTrap::ClapTrap(void) : _name("TNO"), _hit_points(10), _energy_points(10), _attack_damage(0) {
...
ClapTrap::~ClapTrap(void) {
	std::cout << "ClapTrap " << _name << " goes back to sleep." << std::endl;
}
...
void	ClapTrap::takeDamage(unsigned int amount) {
	...
	if (_hit_points > 0)
	{
		std::cout << _name << " has " << _hit_points <<
...
void	ClapTrap::beRepaired(unsigned int amount) {
	...
	if (_energy_points > 0)
	{
		std::cout << _name << " healed itself for " << amount <<
		" hit points!" << std::endl;
		_hit_points += amount;
		std::cout << _name << " now has " << _hit_points << 
		 " hit points left!" << std::endl;
		_energy_points--;
		std::cout << _name << " has " << _energy_points 
		<< " energy points left!" << std::endl;
	}
	else
		std::cout << "ClapTrap " << _name << " has no energy points left!" <<
